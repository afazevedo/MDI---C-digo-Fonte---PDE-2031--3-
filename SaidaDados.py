...
class SaidaDados:
    ...
    def __init__(self, sistema, problema, path, plan_dados, pasta_cod, nomeSubsistemas, habilitaCustoRestPot):
        ...
        # cria o flag 0/1 da consideracao do custo de GT na restricao de potencia
        self.flagCustoRestPot = 1 if habilitaCustoRestPot else 0
            ...
            for isis in (m.subsistemas):
                for ipat in (m.patamares):
                    for iper in (m.periodosTotal):
                        ...
                        saidaResul.write(str(self.sin.subsistemas[isis].montanteRenovExMGD[iper] * self.sin.subsistemas[isis].fatorPatMGD[ipat][iper%12]) + ";");
    ...
    def imprimeSaidaExpansao(self):        
        ...
        saidaResul = open(self.caminho + "saidaExpansaoHidro.txt", "w");
        
        # cabecalho
        saidaResul.write("UHE; Periodo\n");
        ...
        for hidro in modelo.projUHENova:
            ...
            for per in modelo.periodosTotal: # percorre todos os periodos
                if (modelo.investHidro[hidro,per].value == 1): mes_entrada = per  # no periodo que entrou atribui o periodo

            # so imprime se a usina tiver entrado
            if mes_entrada < modelo.sin.numMeses:
                # imprime a entrada
                saidaResul.write(str(int(sin.listaGeralProjUHE[hidro].indexUsinaExterno)) + ";" + str(int(mes_entrada+1)) + "\n");
    ...
    def imprimeSaidaIntercambio(self):
        ...
        # cabecalho
        saidaResul.write("Periodo; Sis_Origem; Sis_Destino; Patamar; CapExistente; ExpansaoInt\n");
            ...
            for isis in modelo.subsistemas:
                for jsis in modelo.subsistemas:
                    for ipat in modelo.patamares:
                        saidaResul.write(str(iper + 1) + ";" + str(isis + 1) + ";" + str(jsis + 1) + ";" + str(ipat + 1) + ";" + str(self.sin.subsistemas[isis].capExistente[jsis][ipat][iper]) + ";" + str(modelo.capExpInter[isis,jsis,iper].value) + "\n");
    ...
    def imprimeResumoExecutivo(self):
        ...
        # encontra a coluna com os dados para o resumo
        coluna = 0;
        while (aba.cell(row=7,column=5).offset(column=coluna).value is not None):
            coluna += 1;
        inicio_dados = coluna + 5;
        ...
        linha = 0;
        while (aba.cell(row=8,column=inicio_dados).offset(row=linha).value is not None):
            # pega o tipo
            tipoProj = aba.cell(row=8,column=inicio_dados).offset(row=linha).value;
            
            # pega os projetos
            projs = aba.cell(row=8,column=inicio_dados+1).offset(row=linha).value;
        ...
        for icen in (m.condicoes):
            ...
            for isis in (m.subsistemas):
                for iper in (m.periodosTotal):
                    ...
                    saidaResul.write(str(m.enMGDEx[isis,iper,icen] * s.subsistemas[isis].fatorPatMGD[0][iper%12]) + ";");
                    saidaResul.write(str(sum(term.potUsina[iper] for term in s.subsistemas[isis].listaTermica)*(1 - self.flagCustoRestPot)
                                        + sum(m.prodTermPot[term.nomeUsina,iper,icen].value for term in s.subsistemas[isis].listaTermica)*self.flagCustoRestPot) + ";");
                    ...
                    saidaResul.write(str(sum(m.capTermCont[proj.nomeUsina,iper].value*proj.potUsina for proj in s.subsistemas[isis].listaProjTermica)*(1 - self.flagCustoRestPot)
                                        + sum(m.prodTermContPot[proj.nomeUsina,iper,icen].value for proj in s.subsistemas[isis].listaProjTermica)*self.flagCustoRestPot) + ";");
    ...
    def imprimeCustosPeriodo(self):
        ...
        for iper in modelo.periodosTotal:
            ...
            custo_tot = modelo.sin.horasMes*(
                        ...
                        sum(modelo.sin.probHidro[icen]*modelo.penalidadeGHMinExist[isis,ipat,iper,icen].value for isis in modelo.subsistemas for ipat in modelo.patamares for icen in modelo.condicoes)*modelo.sin.penalidGHMin + \
                        sum(modelo.sin.probHidro[icen]*modelo.penalidadeGHMinNova[iuhe,ipat,iper,icen].value for iuhe in modelo.projUHENova for ipat in modelo.patamares for icen in modelo.condicoes)*modelo.sin.penalidGHMin + \
            ...
            custo_pen = sum(modelo.sin.probHidro[icen]*modelo.penalidadeGHMinExist[isis,ipat,iper,icen].value for isis in modelo.subsistemas for ipat in modelo.patamares for icen in modelo.condicoes)*modelo.sin.penalidGHMin + \
                        sum(modelo.sin.probHidro[icen]*modelo.penalidadeGHMinNova[iuhe,ipat,iper,icen].value for iuhe in modelo.projUHENova for ipat in modelo.patamares for icen in modelo.condicoes)*modelo.sin.penalidGHMin;
        ...
        for icen in modelo.condicoes:
            ...
            for iper in modelo.periodosTotal:
                ...
                custo_tot = modelo.sin.horasMes*(
                            ...
                            sum(modelo.penalidadeGHMinExist[isis,ipat,iper,icen].value for isis in modelo.subsistemas for ipat in modelo.patamares)*modelo.sin.penalidGHMin + \
                            sum(modelo.penalidadeGHMinNova[iuhe,ipat,iper,icen].value for iuhe in modelo.projUHENova for ipat in modelo.patamares)*modelo.sin.penalidGHMin + \
                ...
                custo_pen = sum(modelo.penalidadeGHMinExist[isis,ipat,iper,icen].value for isis in modelo.subsistemas for ipat in modelo.patamares)*modelo.sin.penalidGHMin + \
                            sum(modelo.penalidadeGHMinNova[iuhe,ipat,iper,icen].value for iuhe in modelo.projUHENova for ipat in modelo.patamares)*modelo.sin.penalidGHMin;
    ...
    def imprimeInfoMDI(self):
        sin = self.sin;

        # abre o arquivo para a saidas
        saidaResul = open(self.caminho + "infoMDI.txt", "w");
        
        # cabecalho
        saidaResul.write("variavel; valor\n");
        
        # dados
        saidaResul.write("inicioHorizonteEstudo; " + str(sin.inicioSim.year) + str("{:02d}".format(sin.inicioSim.month)) + "\n");
        saidaResul.write("fimHorizonteEstudo; " + str(sin.finalSim.year) + str("{:02d}".format(sin.finalSim.month)) + "\n");
        saidaResul.write("periodosHorizonteEstudo; " + str(sin.numMeses));
